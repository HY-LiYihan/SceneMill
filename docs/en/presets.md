...
| Preset | Status | Use |
| --- | --- | --- |
| `images_auto_isaac.yaml` | Stable | Default image entrypoint: fewer than 10 images use AnySplat, otherwise classic. |
...
The `auto` preset uses `router.low_view_threshold: 10` for backend selection. DA3 presets default to `dataset_downsample_factor: 4` and `max_colmap_points: 200000` for 3DGUT stability on high-resolution image captures.
